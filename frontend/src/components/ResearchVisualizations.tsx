...
const ResearchVisualizations: React.FC = () => {
  ...
  useEffect(() => {
    // Load static data generated by our research script
    const loadData = async () => {
      try {
        const [pcaRes, profRes, matRes] = await Promise.all([
          fetch('/data/pca_data.json'),
          fetch('/data/genre_profiles.json'),
          fetch('/data/confusion_matrix.json')
        ]);
        
        setPcaData(await pcaRes.json());
        setProfiles(await profRes.json());
        setMatrixData(await matRes.json());
      } catch (e) {
